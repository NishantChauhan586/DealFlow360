    ...
    async def negotiate_quotation(
        ...
        if negotiation_in.line_changes:
            ...
            for change in negotiation_in.line_changes:
                ...
                target_line.line_total = compute_line_total(
        ...
        if negotiation_in.overall_discount is not None:
            for line in quotation.lines:
                ...
                line.line_total = compute_line_total(
                    line.unit_price,
                    line.quantity,
        ...
        if brs_result.risk_level in [RiskLevel.MEDIUM, RiskLevel.HIGH]:
            ...
            governance_explanation = (
                f"Counter-offer exceeded discount thresholds (BRS Score: {brs_result.score:.1f}, "
                f"{brs_result.risk_level.value.upper()} RISK). "
                f"Quotation has been submitted to {brs_result.required_approval_role} for review."
            )
        else:
            ...
            governance_explanation = (
                f"Counter-offer discount is within authorized tier limits (BRS Score: {brs_result.score:.1f}, "
