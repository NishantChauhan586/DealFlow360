...
class ReportingService:
    ...
    async def generate_sales_report(
        ...
        if not quotes:
            # Fallback to Quote model (table quotes)
            q_stmt = select(Quote).options(selectinload(Quote.items)).where(
                Quote.created_at >= filter_start,
                Quote.created_at <= filter_end,
            ).order_by(Quote.created_at.desc())
            q_res = await self.session.execute(q_stmt)
            seeded_quotes = list(q_res.scalars().all())

            for sq in seeded_quotes:
                gross = sq.subtotal or (sum(l.quantity * l.unit_price for l in sq.items) if sq.items else sq.subtotal)
                disc = sq.discount_amount or 0.0
                net = sq.grand_total or gross - disc

                total_gross += gross
                total_discount += disc
                total_net += net

                categories = list({l.category for l in sq.items if l.category}) if sq.items else ["Hardware"]

                items.append(
                    SalesReportItem(
                        quotation_id=str(sq.id),
                        customer_id=str(sq.customer_name or ""),
                        sales_rep_id=str(sq.sales_rep or ""),
                        status=str(sq.status.value if hasattr(sq.status, 'value') else sq.status or ""),
                        created_at=sq.created_at,
                        total_amount=sq.grand_total,
                        discount_total=sq.discount_amount,
                        blended_risk_score=float(sq.risk_score) if sq.risk_score is not None else None,
                        line_count=len(sq.items) if sq.items else 1,
            ...
            for q in quotes:
                ...
                items.append(
                    SalesReportItem(
                        ...
                        sales_rep_id=str(q.sales_rep_id),
                        status=str(q.status.value if hasattr(q.status, 'value') else q.status or ""),
