...
async def create_warehouse(
    ...
    validated = WarehouseResponse.model_validate(warehouse)
    await publish_event("warehouses", {"action": "create", "warehouse": validated.model_dump(mode="json")})
    return validated
...
async def update_warehouse_inventory(
    ...
    await session.commit()
    validated = InventoryResponse.model_validate(inv)
    await publish_event("warehouses", {"action": "inventory_update", "warehouseId": str(warehouse_id), "inventory": validated.model_dump(mode="json")})
    return validated
