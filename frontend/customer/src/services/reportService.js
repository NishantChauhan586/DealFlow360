...
class ReportService {
  ...
  /**
   * Incorporate any customer-submitted quotes created in the Customer Portal
   */
  _loadCustomerQuotations() {
    try {
      const saved = localStorage.getItem('dealflow_customer_quotations');
      if (saved) {
        const portalQuotes = JSON.parse(saved);
        portalQuotes.forEach((pq) => {
          if (!this.salesRecords.some((r) => r.id === pq.id)) {
            const gross = (pq.subtotal || 0) + (pq.discountAmount || 0);
            const net = pq.grandTotal || pq.subtotal || 0;
            const topItem = pq.lineItems?.[0]?.name || 'Custom Product Bundle';
            this.salesRecords.unshift({
              id: pq.id,
              date: pq.createdAt ? pq.createdAt.split('T')[0] : '2026-09-05',
              customer: pq.companyName || pq.customerEmail || 'Self-Service Client',
              salesRep: 'S. Adeyemi',
              ...
              discountPercent: pq.discountPercent || 0,
              netAmount: net,
              approvalStatus: pq.status === 'Approved' ? 'Approved' : pq.status === 'Pending Review' ? 'Pending Approval' : 'Draft',
              marginPercent: 28.0,
              cycleDays: 2,
            });
          }
  ...
  filterRecords(filters = {}) {
    ...
    const referenceDate = new Date('2026-09-05T23:59:59Z');
    ...
    let results = this.salesRecords.filter((record) => {
      ...
      // 1. Period Filter
      if (period === 'today') {
        if (record.date !== '2026-09-05') return false;
