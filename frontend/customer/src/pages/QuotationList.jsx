  ...
  useEffect(() => {
    ...
    async function loadQuotes() {
      ...
      if (isMounted && Array.isArray(apiQuotes)) {
        setQuotesList(
          apiQuotes.map((q) => {
            const rawVal = typeof q.grandTotal === 'number' ? q.grandTotal : (typeof q.grand_total === 'number' ? q.grand_total : null);
            return {
              id: q.id,
              cust: q.customer_name || q.company_name || q.cust || 'Client',
              amt:
                rawVal !== null
                  ? `₹${Math.round(rawVal).toLocaleString('en-IN')}`
                  : q.amt || '₹0',
              stage: q.status || q.stage || 'Draft',
              rep: q.sales_rep || q.rep || 'Sales Rep',
              items: Array.isArray(q.items) ? q.items.length : 1,
              date: q.submitted_at || q.date || 'Recent',
              risk: q.risk_level || q.risk || 'Low',
            };
          })
        );
      }
