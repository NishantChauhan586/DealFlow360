...
class WarehouseService {
  ...
  _init() {
    ...
    try {
      const existingWh = localStorage.getItem(STORAGE_KEYS.WAREHOUSES);
      const existingOrders = localStorage.getItem(STORAGE_KEYS.ORDERS);
      if (
        (existingWh && (existingWh.includes('wh_main') || existingWh.includes('wh_east'))) ||
        (existingOrders && existingOrders.includes('ORD-4821'))
      ) {
  ...
  _initSyncBus() {
    syncBus.subscribe('warehouses', (payload) => {
      try {
        ...
        console.error('[Admin WarehouseService] Error in sync handler:', err);
      }
      this._notify();
    });
    ...
    syncBus.subscribe('fulfillment_orders', (payload) => {
      try {
        ...
        console.error('[Admin WarehouseService] Error in orders sync handler:', err);
      }
      this._notify();
    });
  }
    ...
    itemsReq.forEach((it) => {
      ...
      if (unfulfilled > 0) {
        backorders.push({
          ...
          targetDepot: warehouses[0]?.name || 'Primary Facility',
        });
      }
    });
