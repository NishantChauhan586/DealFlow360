...
export const calculateMarginPct = (basePrice, costPrice) => {
...
export const calculateGrossMargin = (basePrice, costPrice) => {
  const base = Number(basePrice) || 0;
  const cost = Number(costPrice) || 0;
  return base - cost;
};
...
class ProductService {
  ...
  _initSyncBus() {
    syncBus.subscribe('products', (payload) => {
      try {
        ...
        console.error('[Admin ProductService] Error handling sync event:', err);
      }
      this._notify();
    });
    ...
    syncBus.subscribe('price_lists', (payload) => {
      try {
        ...
        console.error('[Admin ProductService] Error handling price_lists sync:', err);
      }
      this._notify();
    });
    ...
    syncBus.subscribe('discount_tiers', (payload) => {
      try {
        ...
        console.error('[Admin ProductService] Error handling discount_tiers sync:', err);
      }
      this._notify();
    });
  }
  ...
  validateProduct(product, editingId = null) {
    ...
    if (product.basePrice === undefined || product.basePrice === null || product.basePrice === '' || Number(product.basePrice) < 0) {
      errors.basePrice = 'Base Price must be non-negative';
    }
    if (product.costPrice !== undefined && product.costPrice !== null && product.costPrice !== '' && Number(product.costPrice) < 0) {
  ...
  /**
   * Bulk Price Update:
   * targetField: 'basePrice' | 'costPrice'
   * mode: 'PCT_INC' | 'PCT_DEC' | 'FIX_INC' | 'FIX_DEC'
   * value: number
   * category / categoryFilter: string ('All' or specific category)
   * productIds: array of IDs (optional filter)
   */
  bulkUpdatePrices({ targetField = 'basePrice', mode, value, categoryFilter, category = 'All', productIds = null }) {
    const selectedCategory = categoryFilter !== undefined ? categoryFilter : category;
    ...
    const updated = this._getRawProducts().map((p) => {
      ...
      if (matchCategory && matchSelection) {
        const fieldToUpdate = targetField === 'costPrice' ? 'costPrice' : 'basePrice';
        const currentVal = Number(p[fieldToUpdate]) || 0;
        let newVal = currentVal;

        if (mode === 'PCT_INC') newVal = currentVal * (1 + val / 100);
        else if (mode === 'PCT_DEC') newVal = Math.max(0, currentVal * (1 - val / 100));
        else if (mode === 'FIX_INC') newVal = currentVal + val;
        else if (mode === 'FIX_DEC') newVal = Math.max(0, currentVal - val);
        ...
        if (fieldToUpdate === 'basePrice') {
          updatedProduct.variants = (p.variants || []).map((v) => ({
            ...v,
            finalPrice: Math.round((newVal + (Number(v.extraPrice) || 0)) * 100) / 100,
          }));
        }
  ...
  savePriceList(data) {
    if (data.id) {
      return this.updatePriceList(data.id, data);
    } else {
      return this.createPriceList(data);
    }
  }
