...
export default function Fulfillment({ user }) {
  ...
  const refreshData = useCallback(() => {
    ...
    if (whs.length > 0 && (!selectedWarehouseId || !whs.some((w) => w.id === selectedWarehouseId))) {
  ...
  const handleConsolidateBackorder = () => {
    ...
    showToast('Consolidated into a single scheduled shipment landing at primary facility.');
  };
      ...
      {activeTab === 'facilities' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div style={{ background: '#fff', padding: '20px', borderRadius: '4px', border: '1px solid var(--line)' }}>
            ...
            {warehouses.length === 0 ? (
              <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--ink-60)', background: 'var(--paper-2)', borderRadius: '6px' }}>
                <IconTruck style={{ width: 38, height: 38, color: 'var(--ink-40)', marginBottom: '10px' }} />
                <h4 style={{ margin: '0 0 6px 0', fontSize: '15px', color: 'var(--burnham)' }}>No Warehouse Facilities Registered</h4>
                <p style={{ margin: '0 0 16px 0', fontSize: '12.5px', color: 'var(--ink-60)' }}>
                  Click "New Facility" above to create your first warehouse node and configure shipping weights.
                </p>
                {isAdmin && (
                  <button type="button" className="btn btn-dark" onClick={handleOpenCreateWarehouse} style={{ fontSize: '12.5px' }}>
                    <IconPlus style={{ width: 13, height: 13, marginRight: 5 }} /> Create Warehouse Facility
                  </button>
                )}
              </div>
            ) : (
      ...
      {activeTab === 'autosplit' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div className={styles.splitCard}>
            ...
            {orders.length === 0 ? (
              <div style={{ padding: '48px 24px', textAlign: 'center', color: 'var(--ink-60)', background: 'var(--paper-2)', borderRadius: '6px', marginTop: '16px' }}>
                <IconSliders style={{ width: 42, height: 42, color: 'var(--ink-40)', marginBottom: '12px' }} />
                <h3 style={{ margin: '0 0 8px 0', fontSize: '16px', color: 'var(--burnham)' }}>No Fulfillment Orders Available</h3>
                <p style={{ margin: 0, fontSize: '13px', color: 'var(--ink-60)', maxWidth: '480px', marginLeft: 'auto', marginRight: 'auto' }}>
                  There are currently no customer orders available for fulfillment split simulation. Once orders are submitted by you, live multi-warehouse routing and shipping cost optimizations will display automatically.
                </p>
              </div>
            ) : (
              <>
                {/* Split Metrics Summary */}
                {splitResult && activeOrder && (
                  <div
                    style={{
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '18px', fontWeight: 700, color: 'var(--burnham)', marginTop: '2px' }} className="tnum">
                        ${isConsolidated ? (splitResult.totalShippingCost * 0.85).toFixed(2) : splitResult.totalShippingCost.toFixed(2)}
                      </div>
                    </div>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', marginBottom: '24px' }}>
                  <div style={{ fontSize: '13.5px', fontWeight: 700, color: 'var(--burnham)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontSize: '12px', fontWeight: 400, color: 'var(--ink-60)' }}>
                      Total Requested: <strong>{activeOrder?.items ? activeOrder.items.reduce((s, i) => s + (i.requestedQty || 0), 0) : 0} units</strong>
                    </span>
                  </div>

                  {splitResult && activeOrder &&
                    splitResult.allocations.map((alloc, idx) => {
                      const totalReq = activeOrder.items ? activeOrder.items.reduce((sum, it) => sum + (it.requestedQty || 0), 0) : 1;
                      const pct = totalReq > 0 ? Math.round((alloc.allocatedUnits / totalReq) * 100) : 0;
                      const estShipmentCount = alloc.allocatedUnits > 50 ? 2 : 1;
                      const whObj = warehouses.find((w) => w.id === alloc.warehouseId);
                      const baseFee = whObj ? whObj.baseShippingCost : 15;
                        ...
                        >
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px', background: 'var(--paper-2)', padding: '12px 14px', borderRadius: '6px', fontSize: '12px' }}>
                            ...
                            <div>
                              ...
                              <span>Base Fee: ${baseFee.toFixed(2)} + Weight</span>
                            </div>
                          </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '12px', color: 'var(--ink-60)', marginTop: '2px' }}>
                          Stock for remaining <strong>{splitResult.backorders.reduce((sum, b) => sum + b.backorderQty, 0)} backordered units</strong> arrived mid-fulfillment. Consolidate into a single shipment?
                        </div>
                      </div>
                    </div>
                ...
                {isConsolidated && (
                  <div style={{ background: 'rgba(67, 138, 126, 0.12)', border: '1px solid var(--viridian-600)', padding: '14px 20px', borderRadius: '8px', fontSize: '13px', color: 'var(--viridian-600)', fontWeight: 700, marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <span>Consolidated Remaining Backorder: Order merged into 1 single dispatch upon stock receipt! Freight savings applied.</span>
                  </div>
                )}
