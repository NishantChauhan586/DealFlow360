...
export default function ProductManagement({ user }) {
  ...
  const catalogStats = useMemo(() => {
    ...
    const categoriesCount = new Set(all.map((p) => p.category).filter(Boolean)).size;
    const totalMargin = all.reduce((acc, p) => {
      const base = Number(p.basePrice) || 0;
      const cost = Number(p.costPrice) || 0;
      const m = base > 0 ? ((base - cost) / base) * 100 : 0;
      return acc + m;
    }, 0);
    const avgMargin = all.length > 0 ? (totalMargin / all.length).toFixed(1) : '0.0';
    const totalInventoryValueNum = all.reduce((acc, p) => {
    ...
    return {
      ...
      categoriesCount,
      avgMargin,
      totalInventoryValue: formatINR(totalInventoryValueNum),
    };
  }, [productsData]);
      ...
      <div className={styles.headerCard}>
        <div className={styles.headerTop}>
          ...
          <div className={styles.headerActions}>
            ...
            >
              <IconTag style={{ width: 15, height: 15 }} /> Price Lists
            </button>
            <button
              className="btn btn-ghost"
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}
            >
              <IconSliders style={{ width: 15, height: 15 }} /> Bulk Price Update
            </button>
            <button
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}
            >
              <IconPlus style={{ width: 15, height: 15 }} /> + Add Product
            </button>
          </div>
        </div>
      ...
      {activeTab === 'products' && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
            <div style={{ background: '#fff', padding: '16px 20px', borderRadius: '6px', border: '1px solid var(--line)', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ fontSize: '11.5px', color: 'var(--ink-60)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 600 }}>Total Products Count</div>
              <div style={{ fontSize: '24px', fontWeight: 700, color: 'var(--burnham)', marginTop: '6px' }} className="tnum">
                {catalogStats.total}
              </div>
            </div>

            <div style={{ background: '#fff', padding: '16px 20px', borderRadius: '6px', border: '1px solid var(--line)', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ fontSize: '11.5px', color: 'var(--ink-60)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 600 }}>Categories Count</div>
              <div style={{ fontSize: '24px', fontWeight: 700, color: 'var(--burnham)', marginTop: '6px' }} className="tnum">
                {catalogStats.categoriesCount}
              </div>
            </div>

            <div style={{ background: '#fff', padding: '16px 20px', borderRadius: '6px', border: '1px solid var(--line)', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ fontSize: '11.5px', color: 'var(--ink-60)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 600 }}>Average Catalog Margin %</div>
              <div style={{ fontSize: '24px', fontWeight: 700, color: 'var(--viridian-600)', marginTop: '6px' }} className="tnum">
                {catalogStats.avgMargin}%
              </div>
            </div>

            <div style={{ background: '#fff', padding: '16px 20px', borderRadius: '6px', border: '1px solid var(--line)', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
              <div style={{ fontSize: '11.5px', color: 'var(--ink-60)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 600 }}>Total Inventory Value (₹)</div>
              <div style={{ fontSize: '22px', fontWeight: 700, color: 'var(--burnham)', marginTop: '6px' }} className="tnum">
                {catalogStats.totalInventoryValue}
              </div>
            </div>
          </div>
            ...
            <div className={styles.tableWrapper}>
              <table className={styles.catalogTable}>
                ...
                <tbody>
                  {productsData.items.length === 0 ? (
                    <tr>
                      <td colSpan={7} style={{ textAlign: 'center', padding: '48px 20px', color: 'var(--ink-40)' }}>
                    ...
                    productsData.items.map((product) => {
                      ...
                      let inventoryStatusText = 'In Stock';
                      let inventoryStatusTag = 'tag-green';
                      if (stockQty === 0) {
                        inventoryStatusText = 'Out of Stock';
                        inventoryStatusTag = 'tag-red';
                      } else if (stockQty <= threshold) {
                        inventoryStatusText = 'Low Stock';
                        inventoryStatusTag = 'tag-amber';
                      }
                      ...
                      return (
                        <tr key={product.id}>
                          ...
                          <td>
                            <strong className="tnum" style={{ color: 'var(--burnham)', fontSize: '14px' }}>
                              {formatINR(basePrice)}
                            </strong>
                            <div style={{ fontSize: '11px', color: 'var(--ink-40)' }}>per {product.unit || 'Piece'}</div>
                          </td>

                          <td>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              <span className="tnum" style={{ color: 'var(--ink-60)', fontSize: '13px' }}>
                                {formatINR(costPrice)}
                              </span>
                          ...
                          <td>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
                              <span className="tnum" style={{ fontSize: '13px', fontWeight: 600, color: 'var(--burnham)' }}>
                                {stockQty} {product.unit || 'Piece'}{stockQty === 1 ? '' : 's'}
                              </span>
                              <span className={`tag ${inventoryStatusTag}`} style={{ fontSize: '10.5px', width: 'fit-content' }}>
                                {inventoryStatusText}
                              </span>
                            </div>
                          </td>
                    ...
                    {productService.getAllProducts().map((prod) => {
                      ...
                      return (
                        <tr key={prod.id}>
                          ...
                          <td>
                            <strong className="tnum">{formatINR(prod.basePrice)}</strong>
                          </td>
                          {customerTiers.map((tier) => {
